<script type="text/javascript">
	...
	$(document).ready(function(){	
		...
		$('.LikeThis').click(function(e){
			...
			$.post("/system/pro_tick_shop/like.php?postId="+getID, {
		...
		$('.Unlike').click(function(e){
			...
			$.post("/system/pro_tick_shop/unlike.php?postId="+getID, {
...
$p=0;
$user=$_SESSION['mem'];
$sql_t=mysql_query("SELECT * FROM po_tick_shop where user='".$user."'");
while($row_t=mysql_fetch_array($sql_t))
{
    $ok.=$row_t['pro_id'].",";
}
if ($_REQUEST['p']!='') $p=$_REQUEST['p'];
$sql = "select * from user_shop where id in (".$ok."0) order by id desc limit ".$o*$col*$p.",".$o*$col;
$result = @mysql_query($sql,$con);
$total=CountRecord("user_shop","id in (".$ok."0)");?>
<?php if($total=='0')
{?>
