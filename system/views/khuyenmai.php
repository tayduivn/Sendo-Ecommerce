...
$sql_pro = "select * from products where topkhuyenmai > '0'   and status ='0'  $where and name like '%".$namepro."%' order by ".$by." ".$order." limit ".$row*$col*$p.",".$row*$col;
$result = @mysql_query($sql_pro,$con);
$sql_pro1 = "select * from products where topkhuyenmai > '0' and status ='0'   $where and name like '%".$namepro."%' order by ".$by." ".$order." limit ".$row*$col*$p.",".$row*$col;
$result1 = @mysql_query($sql_pro1,$con);
$total=CountRecord("products","topkhuyenmai > '0'  and status ='0'  $where");
...
>SẢM PHẨM ĐANG ĐƯỢC KHUYẾN MÃI GIẢM GIÁ
</div>
         <div style="
    line-height: 20px;
" >
...
<?
	...
	foreach ($ms as $m)
		if ($m[0]!=$_REQUEST['city'])
			echo "<option value=khuyenmaicity.html&city=".$m[0].">".$m[1]."</option>";
		else
			echo "<option selected value=khuyenmaicity.html&city=".$m[0].">".$m[1]."</option>";
?>
	</select>
                    </div>          
                </div>
