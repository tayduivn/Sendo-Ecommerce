...
$sql_pro = "select * from products where  cat in (".$catallsub."0) and status ='0' $where and name like '%".$namepro."%'  order by ".$by." ".$order." limit ".$row*$col*$p.",".$row*$col;
$result = @mysql_query($sql_pro,$con);
$sql_pro1 = "select * from products where cat in (".$catallsub."0) and status ='0'   order by ".$by." ".$order." limit ".$row*$col*$p.",".$row*$col;
$result1 = @mysql_query($sql_pro1,$con);
$total=CountRecord("products","cat in (".$catallsub."0) and status ='0'   $where");
...
<div class="d-product-list-top">
                <div class="d-searchby">
                    ...
                    <input name="home" type="hidden" value="category">
                    <input name="cat" type="hidden" value="<?php echo $_REQUEST['cat'];?>">
                        <div class="d-item">
    ...
    width: 166px;
    height: 30px;
    border: 1px solid #d4c9c9;" value="" placeholder="Nhập tên sản phẩm">
...
<a style="
    color: white;
" href="./<?php echo doidau(mb_strtolower($row_cat['name'],"UTF8"));?>-ct-<?php echo $row_cat['id'];?>.html">Mới cập nhật</a>
</div>
<div style="float:left;line-height:240%;padding-left:10px;padding-right:10px;height:30px;background: #3c8e36;">
<a style="
    color: white;
" href="<?php echo $url[0];?>&by=price_special&order=asc">Giá rẻ nhất</a>
</div>
<div style="float:left;line-height:240%;padding-left:10px;padding-right:10px;height:30px;background: #2d3392;">
<a style="
    color: white;
" href="<?php echo $url[0];?>&by=price_special&order=desc">Giá cao nhất</a>
</div>
