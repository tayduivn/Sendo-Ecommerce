...
if (isset($_POST['danhdautatca'])) {
	...
	$sql1111 = "update thongbao set active='1'  where   user='".$_SESSION['mem']."' ";
		if (mysql_query($sql1111)) {
			
			
				echo "<SCRIPT LANGUAGE='JavaScript'>

    window.location.href='?home=member&act=thongbao&p=$p';
    </SCRIPT>";
		}	
		else {
				echo "<SCRIPT LANGUAGE='JavaScript'>
...
<h3 style="
    ...
    margin-top: 0px;
">THÔNG BÁO      <button type="submit" name="danhdautatca"   style="background: #4CAF50; padding: 2px 9px; color: #fff; display: inline-block;  ">Đánh dấu đã xem tất cả</button>	</h3>
</form>
<?}else{?>
...
<div style="padding: 11px;background: #0c0c69;color: #fff;"><b>Tổng thông báo: </b><? echo mysql_num_rows(mysql_query("SELECT * FROM thongbao where user='".$_SESSION['mem']."' "));?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 
      <span><b>Chưa xem:</b> <? echo mysql_num_rows(mysql_query("SELECT * FROM thongbao where user='".$_SESSION['mem']."' and active=0 "));?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> 
	  <span><b>Đã xem:</b>  <? echo mysql_num_rows(mysql_query("SELECT * FROM thongbao where user='".$_SESSION['mem']."' and active=1 "));?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> 
...
<?php
$sql_help = "select * from thongbao  orders where user='".$_SESSION['mem']."' order by id DESC limit ".$row*$col*$p.",".$row*$col;
...
if (isset($_POST['danhdau'])) {
	...
	$sql1111 = "update thongbao set active='1' where id='".$active."' ";
		if (mysql_query($sql1111)) {
...
<div style="padding: 11px;" ><b>Thông báo ngày: </b><?php echo $row_pro['date'];?>&nbsp;&nbsp;&nbsp;&nbsp;
		<?if($row_pro['active']=="0"){?>	
		     <input name="active" type="hidden" value="<?php echo $row_pro['id'];?>">
     <button type="submit" name="danhdau"   style="background: #4CAF50; padding: 2px 9px; color: #fff; display: inline-block;  ">Đánh dấu đã xem</button>			
					<?}else{?>

     <b style="background: #F44336; padding: 2px 9px; color: #fff; display: inline-block;  ">	Đã xem </b>
 <?}?>
 <span style="float: right;">Số: <b style="color: #161ae2;">#<?php echo $row_pro['id'];?></b>
 </span>
 </div>
...
$param="";
if ($p>1) echo '<a class="buton_timkiem" title="&#272;&#7847;u tiên" href="/?home=member&act=thongbao&p=0">Đầu tiên</a>  ';
if ($p>0) echo '<a class="buton_timkiem" title="&#272;&#7847;u tiên" href="/?home=member&act=thongbao&p='.($p-1).'">Trang trước</a>';
$from=($p-2>0?$p-1:0);
$to=($p+2<$pages?$p+2:$pages);
for ($i=$from;$i<$to;$i++)
{
	if ($i!=$p) echo '<a href="/?home=member&act=thongbao&p='.$i.'">'.($i+1).'</a> ';
	else echo '<a class="active1">'.($i+1).'</a> ';
}
if ($p<$i-1) echo '<a class="buton_timkiem" title="Ti&#7871;p theo" href="/?home=member&act=thongbao&p='.($p+1).'">Trang tiếp</a>';
