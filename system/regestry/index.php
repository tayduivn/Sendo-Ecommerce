...
$(document).ready(function() {
    ...
    $("#user_name_reg").blur(function() {
        ...
        $.ajax({
            ...
            url: "system/regestry/check_reg.php",          // File xử lý dữ liệu được gởi
...
//////
	...
	///	elseif(mysql_num_rows(mysql_query("SELECT user FROM user_shop WHERE user='$reg_user'"))==0)
	 //   {
		//	$err="Mã khuyến mãi không chính xác";
	//	}
	...
	//	}
	  ...
	  //  {
		...
		$date=date("d-m-Y");
		$date1=date("d-m-Y");
$sql="INSERT INTO user_shop(user, pass, fullname, email, phone, address, company, sex, city, mieuta, re_time,pay_time,level_shop,brithday,website,cat_mem,domain,reg_user,tien,tongkhuyenmai,template)
 values('$user','$pass','$fullname','$email','$phone','$address','$company',
'$sex','$city','$mieuta','$date','$date1','$level_shop','$brithday','$website','$nganhhang','$tenmien','$reg_user','$tien','$tien','$template')";
  ...
  $_SESSION['log'] = $user;
   ...
   if($result)
   {
if (sendmail($adminemail,$email,"Thông tin đăng nhập tại http://shopcantho.vn/ của bạn","Đăng nhập tại địa chỉ: http://shopcantho.vn/quantri<br>Tên đăng nhập : ".$user."<br>Mật khẩu : ".$pass."<br>Hỗ trợ kỹ thuật YAHOO CHAT: danastar_vn - Email: hotro@shopcantho.vn - Điện thoại hỗ trợ: <br>- Tại DN : 0983 055 588 - <br>- Tại HN : 0913 550 669 - <br> - Tại HCM: 0987 428 428 <br>Cảm ơn bạn đã đăng ký và sử dụng dich vụ của http://shopcantho.vn/ Mọi vướng kỹ thuật trong quá trình sử dụng bạn vui lòng liên hệ với chúng tôi hỗ trợ 24/7"));
$folder="counter/".$user."";
...
for($i=1;$i<=5;$i++)
	...
	echo "<script>
	 window.alert('Đăng ký Shop thành công')
	window.location='./quantri'</script>";
   }else{
	   $err="KO THANH CONG";
   }
...
<? 	if ($_REQUEST['alert']=='1') { 
 ...
 <div class="d-shoppingcart-content">
            <p class="p-cart-text bold">ĐĂNG KÝ THÀNH CÔNG.GIAN HÀNG CỦA QUÝ KHÁCH ĐÃ ĐƯỢC TẠO <br>Cám ơn Quý khách đã đăng ký với Gianhangso.Com. Chúng tôi đã gửi thông tin đăng ký vào Email của Quý khách.</p>
            <p class="p-cart-text bold"><b>Quý khách có thể xem gian hàng</b> <a title="Xem website" class="a-cart-link" href="http://<?echo $_SESSION['dangnhap']?>.gianhangso.com">TẠI ĐÂY</a></p>
            <p class="p-cart-text bold"><b>Qúy khách có thể quản trị gian hàng</b> <a title="Quản lý website" class="a-cart-link" href="<? echo $_SESSION['tenmien'];?>/quantri">TẠI ĐÂY</a></p>
        </div>
	...
	<p>Quản trị Website: http://shopcantho.vn/quantri Hoặc <? echo $_SESSION['tenmien'];?>/quantri</p>
	<p>Website của bạn: http://shopcantho.vn/<?echo $_SESSION['dangnhap']?> Hoặc <? echo $_SESSION['temien'];?></p>
	...
	<p align="center"><a href='http://shopcantho.vn/'>Nh&#7845;n &#273;ây &#273;&#7875; tr&#7903; v&#7873; trang chính</a></p-->
<?}	
else{?>
<div id="box_reg">
    ...
    <div class="d-contain-content">
                ...
                <table cellspacing="0" cellpadding="6" border="0" style="width:100%;">
				 <tr>
                        ...
                        <td>
                            <input type="text" maxlength="30" style="width:350px;" id="user_name_reg" name="user" placeholder="Nhập tên đăng nhập tài khoản Shop"/>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="password" maxlength="100" style="width:350px;" name="pass"/>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="password" maxlength="100" style="width:350px;"  name="pass1"/>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" maxlength="100" style="width:350px;" id="email" name="email"/>
							 <span class="s-note">Nhập địa chỉ Email của quý khách</b></span>
							  <div class="thongbao1"></div>
                        </td>
                    </tr>
					 <!--tr>
                        <td style="width:160px;text-align:right;"><b><span ></span>Mã Khuyến Mãi:</b></td>
                        <td>
                            <input type="text" maxlength="100" style="width:350px;"  id="user" name="reg_user"/>
							 
							 <span class="s-note">Nhập mã khuyến mãi để nhận được <b>200</b> Lượt UP sản phẩm
							
							 
							 <input id="tien" name="tien" type="hidden" readonly="true" value="<? echo $tien;?>"></b></span>
            ...
            <fieldset class="fs-content">
                <legend>Thông tin gian hàng</legend>
                <table cellspacing="0" cellpadding="6" border="0" style="width:100%;">
                    <!--tr>
                    ...
                    <tr>
                        <td style="width:160px;text-align:right;"><b><span class="s-batbuoc">*</span>Tên Shop :</b></td>
                        <td>
                            <input type="text" maxlength="150" style="width:350px;"  name="company"/>
							 <span class="s-note">Nhập tên Công Ty, Cửa Hàng, Shop ( Đây là tên hiển thị trên toàn hệ thống)</b></span>
							 
                        </td>
                    </tr>
                    <tr>
                        <td style="width:160px;text-align:right;"><b><span class="s-batbuoc">*</span>Điện thoại :</b></td>
                        <td>
                            <input type="text" maxlength="50" style="width:350px;"   name="phone"/>
								 <span class="s-note">Nhập số điện thoại của Shop (Đây là số điện thoại để liên lạc và hiển thị trên toàn hệ thống)</b></span>
                        </td>
                    </tr>
					 <tr>
                        <td style="width:160px;text-align:right;"><b><span class="s-batbuoc">*</span>Tỉnh/Thành phố :</b></td>
...
<?}?>
						    ...
						    <span class="s-note" style="color: red;">Chọn Tỉnh/Thành Shop đang hoạt động kinh doanh </b></span>
                  
					   </td>
						
                    </tr>
                    <tr>
                        <td style="width:160px;text-align:right;"><b><span class="s-batbuoc">*</span>Địa chỉ :</b></td>
                        <td>
                            <input type="text" maxlength="100" style="width:350px;"  name="address"/>
								 <span class="s-note">Địa chỉ Công Ty, cửa hàng, shop...(Nhập chính xác địa chỉ để thuận tiện cho việc nhận và giao hàng)</b></span>
                        </td>
                    </tr>
...
<input type="submit" name="Save" class="btndk"  value="Mở Shop">
</td></tr>
                </table>
            </fieldset>
        </div>                       
...
function popUp(){
   if (wi) wi.close();
   wi=window.open("system/regestry/popup.php","","width=890,height=500,location=yes,scrollbars=yes,status=yes");
}
