...
<?php
$truy_xuat=mysql_query("SELECT * FROM orders where orders_id='".$_SESSION['tim_id']."'");
$truy_xuat_in=mysql_fetch_assoc($truy_xuat);



if($_SESSION['mem']==$truy_xuat_in['user_mem'] && $_SESSION['tim_id'] == $truy_xuat_in['orders_id']  ){?>
...
if(document.cartform2.phivanchuyen.value =="")
{
alert("Vui lòng chọn Tỉnh/ Thành nhận hàng");
document.cartform2.phivanchuyen.focus();
return false;
}
...
if (isset($_POST['butsub_chuyen'])) {
	...
	$diemlua_action = $_POST['diemlua_action'];
	
	$ordersdetail_product_id = $_POST['ordersdetail_product_id'];
	$ordersdetail_price = $_POST['ordersdetail_price'];

	{
	$sql = "insert into orders (orders_customer_id,orders_name,orders_phone,orders_email,orders_address,orders_user,orders_date,mausac,kichthuoc,user_mem,diemlua,tinhthanh) values ('".$cust['id']."','".$cust['fullname']."','".$cust['phone']."','".$cust['email']."','".$cust['address']."' ,'".$name_order."','".$date."' ,'".$mausac."','".$kichthuoc."' ,'".$_SESSION['mem']."','".$diemlua_action."','0'   )";
		if (mysql_query($sql,$con)) {

	
			{
				$newid=mysql_insert_id();
				$sqlUpdate1 = "insert into orderdetail (ordersdetail_product_id,ordersdetail_quantity,ordersdetail_price,ordersdetail_ordersid,user,user_mem,phivanchuyen) values (".$ordersdetail_product_id.",".$update_soluong.",'".$ordersdetail_price."',".$newid.",'".$name_order."','".$_SESSION['mem']."' ,'0')";
...
if (isset($_POST['butSub'])) {
	...
	$nguoimua = "update products set nguoimua = + 1  where id='".$row999911['ordersdetail_product_id']."' ";
      ...
      window.location.href='?home=quanlydonhang_muachitiet';
    </SCRIPT>";
		}	
		else {
				echo "<SCRIPT LANGUAGE='JavaScript'>
...
<form method="POST"  name="cartform2" onsubmit="return check2()" >	
...
<label > Vận chuyển đến <b ><select   style="border: 1px solid #ddd;    width: 90px;" name="phivanchuyen"  onchange="this.form.submit()" >    
...
<?}?>	</option>         
  	...
  	<?php if($row99991111['city'] =='69' )
{?>   
      <option value="68" title="An Giang">An Giang</option>
...
<?}?>	</option> 															 
	...
	<?}?>	
</select>	</b>chi phí<b style="color: #10ab16;">
                    ...
                    <table class="tableSimple" cellpadding="1" cellspacing="0">
						...
						<tr style="">
                            <td class="tdTextBox" colspan="2">
                                <input  name="name" type="text" value="<? echo $row99991['orders_name']; ?>" maxlength="100" id="txtCompany" placeholder="Họ và Tên người nhận">
								 
                            </td>
                        </tr>
                             <tr style="">
                            <td class="tdTextBox" colspan="2">
                                <input  name="phone" type="text" value="<? echo $row99991['orders_phone']; ?>" maxlength="100" id="txtCompany" placeholder="Số điện thoại">
								 
                            </td>
                        </tr><!--tr style="">
                            <td class="tdTextBox" colspan="2">
                                <input  name="email" type="text" value="<? echo $row99991['orders_email']; ?>" maxlength="100" id="txtCompany" placeholder="Địa chỉ Email">
								 
                            </td>
                        </tr-->
						<tr style="">
                            <td class="tdTextBox" colspan="2">
                                <textarea  style="width: 100%;"  name="address" type="text" value="<? echo $row99991['orders_address']; ?>" maxlength="100" id="txtCompany" placeholder="Địa chỉ nhận hàng"><? echo $row99991['orders_address']; ?></textarea>
								 
                            </td>
                        </tr><tr style="">
                            <td class="tdTextBox" colspan="2">
                                <input  name="ghichu" type="text" value="<? echo $row99991['ghichu']; ?>" maxlength="100" id="txtCompany" placeholder="Ghi chú thêm">
                        ...
                        <tr>
                            <td class="td-register-button" colspan="2">
                                <input name="butSub" type="submit" id="reg-button" href="javascript:" onclick="return CheckRegFormSimple();" value="Mua hàng"/>
                            </td>
                        </tr>
                    </tbody></table>
                </div>
