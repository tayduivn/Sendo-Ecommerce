<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Giá: <b> <a href="./?act=price_m&page=<? echo $_REQUEST['page']; ?>"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$news=GetPriceInfo($id);
			if ($news)
			{
				@$result = mysql_query("delete from price where dv_id=".$id,$con);
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from price where $where","./?act=price&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
    ...
    <td align="center" nowrap class="title"><b>Tên Giá</b></td>
     <td align="center" nowrap class="title"><b>Giá</b></td>

  </tr>
  
  <?
            	$sql="select * from price where $where limit ".($p*$MAXPAGE).",".$MAXPAGE;
  ...
  <tr>
    ...
    <a href="./?act=price_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['dv_id']; ?>">S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
