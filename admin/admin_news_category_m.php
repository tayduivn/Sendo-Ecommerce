...
if (isset($_POST['butSaveLoai'])) {
	...
	else
	{
		if (!empty($_POST['oldid'])) {
			$oldid = $_POST['oldid'];
			$sql = "update news_cat set name='".$name."', parent='".$parent."', thutu='".$thutu."', status='".$status."' where id='".$oldid."'";
		}
		else 
			$sql = "insert into news_cat (name,parent,status,thutu,date) values ('".$name."','".$parent."','".$status."','".$thutu."',SYSDATE())";
...
<?
	if (isset($_GET['id'])) {
		$oldid=$_GET['id'];
		$sql = "select * from news_cat where id='".$oldid."'";
