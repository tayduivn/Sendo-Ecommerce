<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Currency : <b> <a href="./?act=currency_m"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from currency where id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from currency","./?act=currency"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
    ...
    <td align="center" nowrap class="title"><b>Tên tiền</b></td>
    <td align="center" nowrap class="title"><b>Quy đổi</b></td>
    <td align="center" nowrap class="title"><b>Thứ tự</b></td>
  </tr>
  
  <?
            	$sql="select * from currency order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=currency_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=currency&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">Xoá</a></td>
    ...
    <td width="80" bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['conver']; ?>&nbsp;</td>
