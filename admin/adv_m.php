...
if (isset($_POST['butSaveLoai'])) {
	...
	if (!empty($_POST['id'])) {
			$oldid = $_POST['id'];
			$sql = "update products set code='".$code."', trogia='".$trogia."',  trogia_link='".$link."', name='".$name."', price='".$price."',price_special='".$priceen."', short='".$shortdesc."', content='".$desc."', cat='".$cat."', thutu='".$sort."' where id='".$oldid."'";
		}else {
			$sql = "insert into products (code,trogia,trogia_link, name, price, price_special, short, content , cat, thutu) values ('".$code."','".$trogia."', '".$link."','".$name."','".$price."','".$priceen."','".$shortdesc."','".$desc."','".$cate."','".$sort."')";
		}
		//echo $sql;
		//exit();
  	...
  	if ($err=='') echo '<script>window.location="index.php?act=product&status='.$_REQUEST['status'].'&cat='.$_REQUEST['cat'].'&page='.$_REQUEST['page'].'&code=1"</script>';
...
?>		
     ...
     <tr>
        <td width="15%" class="smallfont">
        <p align="right">Link đến sản phẩm trợ giá</td>
        <td width="1%" class="smallfont" align="center">
        <font color="#FF0000">*</font></td>
        <td width="83%" class="smallfont">
		<INPUT value="<? echo $link; ?>" TYPE="text" NAME="link" CLASS=textbox size="89"></td>
      </tr>
     ...
     <tr>
        ...
        &nbsp;</td>
        <td width="83%" class="smallfont">
<textarea style="width:462; height:99" name="txtShortDesc" cols="4" cols="28"><? echo $shortdesc; ?></textarea>
		</td>
      </tr>
...
?>		
	   ...
	   <tr>
        <td width="15%" class="smallfont" align="right">
        Là sản phẩm trợ giá</td>
        <td width="1%" class="smallfont" align="right">&nbsp;
        </td>
        <td width="83%" class="smallfont">
		<select name="trogia">
		<?if($row['trogia']=='0'){?>
		<option value="0" selected>Không là sản phẩm trợ giá</option>
		<option value="1">Là sản phẩm trợ giá</option>
		<?}else{?>
		<option value="1" selected>Là sản phẩm trợ giá</option>
		<option value="0">Không là sản phẩm trợ giá</option>
		<?}?>
     ...
     <tr>
        ...
        <td width="83%" class="smallfont">
		<INPUT value="<? echo $thutu; ?>" TYPE="text" NAME="txtSortOrder" CLASS=textbox size="20"></td>
      </tr>
      <tr>
        <td width="15%" class="smallfont">
		<p align="right">
