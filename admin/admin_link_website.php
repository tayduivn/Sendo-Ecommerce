	...
	switch ($_GET['action'])
	{
		case 'del' :
			...
			if ($pro)
			{
				...
				if ($result) 
				{
					if (file_exists($link.$pro['image'])) unlink($link.$pro['link_websites_img']);
...
<?
	if (isset($_POST['ButDel'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($pro)
			{
				@$result = mysql_query("delete from ads where id='".$id."'",$con);
				if ($result) {
					$cnt++;
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from ads","./?act=link_website"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?php
           	$sql="select * from ads order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
  ...
  <tr>
    ...
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=link_website_m&id=<? echo $row['id']; ?>&page=<? echo $_REQUEST['page'];?>">S&#7917;a</a></td>
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=link_website&page=<? echo $_REQUEST['page']; ?>&action=del&id=<? echo $row['id']; ?>">Xóa</a></td>
	...
	{?>
	Trang chủ header
    <?}elseif($row['cat_id']=='1000000009')
	{?>
	Trang chủ bottom
	<?}elseif($row['cat_id']=='1000000000')
	{?>
	Trang chủ right 1
	<?}elseif($row['cat_id']=='1000000001')
	{?>
	Trang chủ right 2
	<?}elseif($row['cat_id']=='1000000002')
	{?>
	Rao vặt
	<?}elseif($row['cat_id']=='1000000003')
	{?>
	Gian hàng
	<?}elseif($row['cat_id']=='1000000004')
	{?>
	Tuyển dụng
	<?}elseif($row['cat_id']=='1000000005')
	{?>
	Danh bạ doanh nghiệp
	<?}elseif($row['cat_id']=='1000000006')
	{?>
	Chi tiết sản phẩm
	<?}elseif($row['cat_id']=='1000000007')
	{?>
	Products relate
	<?}elseif($row['cat_id']=='1000000008')
	{?>
	Products Vip/New
	<?}else
	{?>
