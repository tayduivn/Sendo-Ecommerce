...
<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        <td class="title" width="100%">Tuyển dụng :
	</td>
      </tr>
    </table>
<?
...
<?
	if (isset($_POST['vip'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			$sql_news=mysql_query("SELECT * FROM job where id='".$id."' ");
			$news=mysql_fetch_assoc($sql_news);

			if ($news)
			{
				@$result = mysql_query("update job set vip=1 where id='".$id."' ",$con);
				if ($result) {
					$cnt++;
		...
		echo "<p align=center class='err'>Đã thêm ".$cnt." vào rao vặt vip</p>";
	}
?>
...
<?
	if (isset($_POST['un_vip'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($news)
			{
				@$result = mysql_query("update job set vip=0 where id='".$id."' ",$con);
				if ($result) {
					$cnt++;
		...
		echo "<p align=center class='err'>Đã hủy ".$cnt." tin rao vặt vip</p>";
	}
?>
...
{	global $con;
	...
	$pageindex=taotrang(CountRecord("job",$where),"./?act=job&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page);
?>
...
?>
	...
	<input type="button" value="Chuy&#7875;n" class="button" onclick="window.location='./?act=job&cat='+ddCat.value">
	</td>
</tr>
</table>
</td>
...
<td>
<input type="hidden" name="act" value="search_job">
<input type="hidden" name="job" value="search_job">
...
<td align="center" nowrap class="title"><b>Ngày đăng</b></td>
  ...
  <?php
           	$sql="select * from job where $where order by id DESC limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
			...
			$catinfo=GetCatJobInfo($row['job_cat']);
...
<input type="hidden" name="act" value="job"><table border="0" width="100%" cellspacing="0" cellpadding="0" id="table1">
	<tr>
		<td>
...
<td align="right">
<input type="submit" value="Ch&#7885;n là Vip" name="vip" class="button">
<input type="submit" value="Hủy tin Vip" name="un_vip" class="button">
</td>
