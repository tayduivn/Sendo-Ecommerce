<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Danh mục danh bạ doanh nghiệp : <b> <a href="./?act=cat_company_m&page=<? echo $_REQUEST['page']; ?>&cat=<? echo $_REQUEST['cat']; ?>"><font color="#FFFFFF">Nh&#7853;p 
...
$MAXPAGE=20;
	...
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$result = mysql_query("select company_cat from company where company_cat='".$id."'",$con);
			if (mysql_num_rows($result)<=0) {
				$sql = "delete from company_cat where id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from company_cat where id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from company_cat where $where","./?act=cat_company&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page);
?>
...
?>
	...
	<input type="button" value="Chuy&#7875;n" class="button" onclick="window.location='./?act=cat_company&cat='+ddCat.value">
	</td>
</tr>
</table>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select * from company_cat where $where order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=cat_job_m&cat=<? echo $_REQUEST['cat']; ?>&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">
	S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=cat_job&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">
	Xoá</a></td>
...
<input type="hidden" name="act" value="cat_company">
</form>
