...
if (isset($_POST['butSaveLoai'])) {
	...
	else
	{
		...
		if (!empty($_POST['oldid'])) {
			$oldid = $_POST['oldid'];
			$sql = "update adv_cat set name='".$CategoryName."', parent='".$parent."', thutu='".$order."' where id='".$oldid."'";
		}
		else 
			$sql = "insert into adv_cat (name, parent, thutu) values ('".$CategoryName."','".$parent."','".$order."')";
		if (mysql_query($sql,$con)) {
			echo "<p align=center class='err'>&#272;ã c&#7853;p nh&#7853;t thành công</p><br>";
			echo "<script>window.location='index.php?act=cat_adv&cat=".$_REQUEST['cat']."&page=".$_REQUEST['page']."'</script>";
		}
...
<?
	if (isset($_GET['id'])) {
		$oldid=$_GET['id'];
		$sql = "select * from adv_cat where id='".$oldid."'";
...
<input type=hidden name="act" value="cat_adv_m">
...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#0069A8" width="100%" id="AutoNumber1">
  <tr>
    <td width="45%" align="center" class="title" height="20">Thêm m&#7899;i / C&#7853;p nh&#7853;t : Danh m&#7909;c Rao vặt</td>
  </tr>
  <tr>
    <td width="45%">
