...
$catallsub=GetCatCompany($cat);
	...
	if ($_REQUEST['cat_id']!='')	$where.=" and company_cat in (".$catallsub."0)";
	...
	$result = mysql_query("select count(*) from company where $where ",$con);
	$total=mysql_fetch_row($result);

	$sql="select * from company where $where limit ".$p*$MAXPAGE.",".$MAXPAGE;
  ...
  <tr>
	...
	<td bgcolor="<? echo $color; ?>" class="smallfont"><a href="../?home=company&act=views&id=<? echo $row['id']; ?>" target="_blank"><? echo $row['name']; ?></a></td>
