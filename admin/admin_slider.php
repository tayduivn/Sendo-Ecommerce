<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        <td class="title" width="100%">Slider : <a href="./?act=slider_m&page=<? echo $_REQUEST['page']; ?>"><font class="V10pt" color="#ffffff">Nh&#7853;p M&#7899;i</font></a>&nbsp;&nbsp;
	</td>
      </tr>
    </table>
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$sql="select * from slider_show_home where id=$id limit 1";
        	$pro=mysql_fetch_assoc(mysql_query($sql,$con));
			$link="../";
			if ($pro)
			{
				$sql = "delete from slider_show_home where id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($pro)
			{
				@$result = mysql_query("delete from slider_show_home where id='".$id."'",$con);
				if ($result) {
					$cnt++;
					if (file_exists($link.$pro['image'])) unlink($link.$pro['image']);
					if (file_exists($link.$pro['image_large'])) unlink($link.$pro['image_large']);
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from slider_show_home","./?act=slider"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?php
           	$sql="select * from slider_show_home order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
  ...
  <tr>
    ...
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=slider_m&id=<? echo $row['id']; ?>&page=<? echo $_REQUEST['page'];?>">S&#7917;a</a></td>
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=slider&page=<? echo $_REQUEST['page']; ?>&action=del&id=<? echo $row['id']; ?>">Xóa</a></td>
