...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select *,DATE_FORMAT(providers_dateadded,'%d/%m/%Y %h:%m') as dateformat from providers where providers_status=0 order by providers_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=provider_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['providers_id']; ?>">
	S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=provider&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['providers_id']; ?>">
	Xoá</a></td>
    ...
    <td bgcolor="<? echo $color; ?>" class="smallfont"><? echo $row['providers_image']; ?>&nbsp;</td>
