...
if (isset($_POST['butSaveLoai'])) {
	...
	$err.=CheckUpload($_FILES["txtFile"],".doc;.xls;.zip;.rar;.pdf;.wma;.wmv;.mp3;.mp4;.asf;.avi;.mpg;.mpeg;",100*1024*1024,$_POST['id']==''?1:0);
	
	if ($err=='')
	{
		if (!empty($_POST['id'])) {
			$oldid = $_POST['id'];
			$sql = "update tailieu set doc_files_name='".$name."',doc_files_description='".$desc."',doc_files_status='".$status."',doc_files_categoriesid='".$categories_id."' where doc_files_id='".$oldid."'";
		}else {
			$sql = "insert into tailieu (doc_files_name,doc_files_description,doc_files_status,doc_files_dateadded,doc_files_categoriesid) values ('".$name."','".$desc."','".$status."',SYSDATE(),'".$categories_id."')";
		}
		...
		if (mysql_query($sql,$con)) {
			...
			{
				@chmod("$path/tai_lieu$oldid$extsmall", 0777);
				$sqlUpdateField = " doc_files_file='$pathdb/$fn' ";
			}

			if($sqlUpdateField!='')
			{
				$sqlUpdate = "update tailieu set $sqlUpdateField where doc_files_id='".$oldid."'";
  	...
  	if ($err=='') echo '<script>window.location="index.php?act=tailieu&page='.$_REQUEST['page'].'&code=1"</script>';
...
<?
	if (isset($_GET['id'])) {
		$oldid=$_GET['id'];
		$page = $_GET['page'];
		$sql = "select * from tailieu where doc_files_id='".$oldid."' limit 1";
...
<input type="hidden" name="act" value="tailieu_m">
