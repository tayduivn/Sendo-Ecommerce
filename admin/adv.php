...
<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        <td class="title" width="100%">Rao vặt :
	</td>
      </tr>
    </table>
<?
	switch ($_GET['action'])
	{
		case 'del' :
			...
			if ($pro)
			{
				$sql = "delete from avd where id='".$id."'";
...
<?
	if (isset($_POST['vip'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			$sql_news=mysql_query("SELECT * FROM avd where id='".$id."' ");
			$news=mysql_fetch_assoc($sql_news);

			if ($news)
			{
				@$result = mysql_query("update avd set vip=1 where id='".$id."' ",$con);
				if ($result) {
					$cnt++;
...
<?
	if (isset($_POST['un_vip'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($news)
			{
				@$result = mysql_query("update avd set vip=0 where id='".$id."' ",$con);
				if ($result) {
					$cnt++;
...
{	global $con;
	...
	$pageindex=taotrang(CountRecord("avd",$where),"./?act=adv&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page);
?>
...
<td>
<input type="hidden" name="act" value="search_adv">
<input type="hidden" name="adv" value="search_adv">
...
<td align="center" nowrap class="title"><b>Ngày đăng</b></td>
  ...
  <?php
           	$sql="select * from avd where $where order by id DESC limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
