...
if (isset($_POST['butSaveLoai'])) {
	...
	else
	{
		if (!empty($_POST['oldid'])) {
			$oldid = $_POST['oldid'];
			$sql = "update news_hot set news_id='".$products_id."', ns_sortorder='".$sort."', ns_status=".$status." where ns_id='".$oldid."'";
		}
		else 
			$sql = "insert into news_hot (news_id, ns_sortorder, ns_status) values ('".$products_id."','".$sort."',".$status.")";
		if (mysql_query($sql,$con)) {
			echo "<p align=center class='err'>&#272;ã c&#7853;p nh&#7853;t thành công</p><br>";
			echo "<script>window.location='index.php?act=news_hot&page=".$_REQUEST['page']."'</script>";
		}
...
<?
	if (isset($_GET['id'])) {
		$oldid=$_GET['id'];
		$sql = "select * from news_hot where ns_id='".$oldid."'";
		if ($result = mysql_query($sql,$con)) {
			$row=mysql_fetch_array($result);
			$products_id=$row['news_id'];
			$order=$row['ns_sortorder'];
			$status=$row['ns_status'];
			$date=$row['ns_dateadded'];
		}
	}
?>

<form method="POST" name="FormLoaiSP" action="index.php">
<input type=hidden name="act" value="news_hot_m">
