<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        <td class="title" width="100%">Th&#432; vi&#7879;n&nbsp; : <a href="./?act=edoc_m&page=<? echo $_REQUEST['page']; ?>"><font class="V10pt" color="#ffffff">Nh&#7853;p M&#7899;i</font></a>
	</td>
      </tr>
    </table>
<?
...
{	global $con;
	...
	$pageindex=taotrang(CountRecord("doc_files",$where),"./?act=edoc&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page);
?>
...
?>
	...
	<input type="button" value="Chuy&#7875;n" class="button" onclick="window.location='./?act=edoc&cat='+ddCat.value">
	</td>
</tr>
</table>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?php
           	$sql="select * from doc_files where $where order by doc_files_id limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
