<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Danh mục tuyển dụng : <b> <a href="./?act=cat_job_m&page=<? echo $_REQUEST['page']; ?>&cat=<? echo $_REQUEST['cat']; ?>"><font color="#FFFFFF">Nh&#7853;p 
...
$MAXPAGE=20;
	...
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$result = mysql_query("select job_cat from job where job_cat='".$id."'",$con);
			if (mysql_num_rows($result)<=0) {
				$sql = "delete from job_cat where id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from job_cat where id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from job_cat where $where","./?act=cat_job&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page);
?>
...
?>
	...
	<input type="button" value="Chuy&#7875;n" class="button" onclick="window.location='./?act=cat_job&cat='+ddCat.value">
	</td>
</tr>
</table>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  ...
  <?
            	$sql="select * from job_cat where $where order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
...
<input type="hidden" name="act" value="cat_job">
</form>
