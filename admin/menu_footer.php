<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        ...
        Menu footer : <b> <a href="./?act=menu_footer_m"><font color="#FFFFFF">Nh&#7853;p 
...
<?
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			$sql = "delete from menu_footer where id='".$id."'";
...
<?
	if (isset($_POST['ButDel'])) {
		...
		{
			@$result = mysql_query("delete from menu_footer where id='".$id."'",$con);
			if ($result) $cnt++;
		}
...
<input type="hidden" name="act" value="menu_footer">
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from menu_footer","./?act=menu_footer"."&page=",$MAXPAGE,$page);
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
    ...
    <td align="center" nowrap class="title"><b>Tên menu</b></td>
    <td align="center" nowrap class="title"><b>Link</b></td>
    <td align="center" nowrap class="title"><b>Thu tu</b></td>
  </tr>
  
  <?
            	$sql="select * from menu_footer order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
            	$result=mysql_query($sql,$con);
            	$i=0;
  ...
  <tr>
    ...
    <a href="./?act=menu_footer_m&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">S&#7917;a</a></td>
    <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=menu_footer&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">Xoá</a></td>
