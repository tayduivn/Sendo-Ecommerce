...
<?
	if (isset($_POST['ButDel'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($news)
			{
				@$result = mysql_query("delete from news where id='".$id."'",$con);
				if ($result) {
					$cnt++;
...
<?
	if (isset($_POST['newstop'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			$sql_news=mysql_query("SELECT * FROM news where id='".$id."' ");
			$news=mysql_fetch_assoc($sql_news);

			if ($news)
			{
				@$result = mysql_query("update news set top=1 where id='".$id."' ",$con);
				if ($result) {
					$cnt++;
...
<?
	if (isset($_POST['un_newstop'])) {
		...
		foreach ($_POST['chk'] as $id)
		{
			...
			if ($news)
			{
				@$result = mysql_query("update news set top=0 where id='".$id."' ",$con);
				if ($result) {
					$cnt++;
		...
		echo "<p align=center class='err'>Đã hủy ".$cnt." tin top</p>";
	}
?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" bordercolor="#C9C9C9" width="100%" id="AutoNumber1">
  <tr>
	...
	<td align="center" nowrap class="title"><a class="title" href="<? echo GetLinkSort(3); ?>"><b>Top</b></a></td>
  ...
  <?
  			...
  			$direction=($_REQUEST['direction']==''||$_REQUEST['direction']=='0'?"desc":"");
           	$sql="select * from news where $where and user='' $sortby $direction limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
			...
			$sql_cat=mysql_query("SELECT * FROM news_cat where id='".$row['cat_id']."' ");
  ...
  <tr>
    ...
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=news_m&cat=<? echo $_REQUEST['cat']; ?>&id=<? echo $row['id']; ?>&page=<? echo $_REQUEST['page'];?>">S&#7917;a</a></td>
    <td align="center" bgcolor="<? echo $color; ?>" class="smallfont">
    <a onclick="return confirm('B&#7841;n có ch&#7855;c ch&#7855;n mu&#7889;n xoá ?');" href="./?act=news&action=del&cat=<? echo $_REQUEST['cat']; ?>&id=<? echo $row['id']; ?>">Xóa</a></td>
	...
	<td bgcolor="<? echo $color; ?>" class="smallfont"><? if($row['top']=='1'){?><font color="blue">Yes</font><?}else{?><font color="red">No</font><?}?></td>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0" id="table1">
	<tr>
	   ...
	   <td align="right">
		<input type="submit" value="Ch&#7885;n là tin top" name="newstop" class="button">
		<input type="submit" value="Hủy tin top" name="un_newstop" class="button">
		</td>
	</tr>
</table>
</form>
