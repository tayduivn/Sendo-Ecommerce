...
Danh sách thành viên mua bán | <a href="?act=users_add"><font color="#ffffff">Thên User</font></a>
</td></tr>
<tr><td>
<?
	switch ($_GET['action'])
...
<input type="hidden" name="act" value="users_sell">
...
{	global $con;
	...
	$pageindex=taotrang("select count(*) from user_shop where level_shop=2","./?act=users_sell"."&page=",$MAXPAGE,$page);
?>
...
<? $sql=mysql_query("SELECT * FROM user_shop where level_shop=2 order by id desc");
$i=0;
...
<td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
	...
	S&#7917;a</a></td>
 <td width="20" bgcolor="<? echo $color; ?>" class="smallfont">
    <a href="./?act=users_sell&action=del&page=<? echo $_REQUEST['page']; ?>&id=<? echo $row['id']; ?>">
	Xoá</a></td>
<td><? echo $row['id'];?></td>
<td><? echo $row['user'];?></td>
<td><? echo $row['pass'];?></td>
...
else echo "<font color=blue>Thành viên mua bán</font>";
?></td>
