...
<table height="28" cellSpacing="0" cellPadding="0" width="100%" border="0">
      <tr align=center>
        <td class="title" width="100%">S&#7843;n ph&#7849;m : <a href="./?act=product_m&page=<? echo $page?>&cat=<? echo $_REQUEST['cat']; ?>"><font class="V10pt" color="#ffffff">Nh&#7853;p M&#7899;i</font></a>&nbsp;&nbsp;
	</td>
      </tr>
    </table>
<?
		...
		case 'del' :
			...
			if ($pro)
			{
				...
				if ($result) 
				{
					...
					mysql_query("delete from pro_new where id='".$id."'",$con);
...
<td align="center" nowrap class="title"><b>Ngày đăng</b></td>
  ...
  <?php
           	$sql="select * from products where $where and cat_mem!=45 and cat_mem!=46 and trogia=1 order by id limit ".($p*$MAXPAGE).",".$MAXPAGE;
        	$result=mysql_query($sql,$con);
        	$i=0;
