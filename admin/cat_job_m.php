...
if (isset($_POST['butSaveLoai'])) {
	...
	else
	{
		...
		if (!empty($_POST['oldid'])) {
			$oldid = $_POST['oldid'];
			$sql = "update job_cat set name='".$CategoryName."', parent='".$parent."', thutu='".$order."' where id='".$oldid."'";
		}
		else 
			$sql = "insert into job_cat (name, parent, thutu) values ('".$CategoryName."','".$parent."','".$order."')";
		if (mysql_query($sql,$con)) {
			echo "<p align=center class='err'>&#272;ã c&#7853;p nh&#7853;t thành công</p><br>";
			echo "<script>window.location='index.php?act=cat_job&cat=".$_REQUEST['cat']."&page=".$_REQUEST['page']."'</script>";
		}
...
<?
	if (isset($_GET['id'])) {
		$oldid=$_GET['id'];
		$sql = "select * from job_cat where id='".$oldid."'";
...
<input type=hidden name="act" value="cat_job_m">
